...
type
  ...
  RawNimNode* = distinct NimNode
    ## nimnode that stays the same during serialization
...
proc read*(format: DeglazeFormat, node: NimNode, val: var NimNode) =
  if node.kind in nnkCallKinds and node.len == 2 and node[0].eqIdent"getCachedNode" and node[1].kind in {nnkIntLit..nnkUInt64Lit}:
    val = nimNodeLitCache[int(node[1].intVal)]
  else:
    glazeError "expected nimnode literal generated by glaze, got " & $node.kind, node

proc marshal*(format: GlazeFormat, val: RawNimNode): NimNode =
  result = NimNode(val)

proc read*(format: DeglazeFormat, node: NimNode, val: var RawNimNode) =
  val = RawNimNode(node)
